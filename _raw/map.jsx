...
/* ==========================================================
   Americas connection diagram
   A clean stylized visualization of LATAM → US engineering flow.
   Two columns of cities with animated arc connections.
   No fake cartography — readable, branded, on-message.
   ========================================================== */
function LatamUSMap({ variant = "light", animated = true }) {
  const isDark = variant === "dark";

  // Two vertical columns: U.S. (top) and LATAM (bottom)
  // viewBox 1200 x 760
  const usHubs = [
  ...
  return (
    ...
    >
      ...
      {/* eyebrows */}
      <g>
        <text x="60" y="78" fontFamily="JetBrains Mono, monospace" fontSize="11" fontWeight="500" letterSpacing="0.18em" fill={eyebrowColor}>
          ◇ U.S. — HIRING TEAMS
        </text>
        <line x1="60" y1="92" x2="1140" y2="92" stroke={dividerColor} strokeWidth="1" />
        <text x="60" y="730" fontFamily="JetBrains Mono, monospace" fontSize="11" fontWeight="500" letterSpacing="0.18em" fill={eyebrowColor}>
      ...
      {arcs.map((a, i) => {
        ...
        if (!f || !t) return null;
        // bezier: from LATAM up to US. control points pull toward middle vertical.
        const c1x = f.x;
      ...
      {usHubs.map(h => (
        <g key={h.id}>
          ...
          <circle cx={h.x} cy={h.y} r="6" fill={usDotFill} stroke={isDark ? "var(--navy-1000)" : "white"} strokeWidth="2" />
          <text x={h.x} y={h.y - 22} textAnchor="middle"
            fontFamily="Oswald, sans-serif" fontSize="22" fontWeight="600" letterSpacing="0.04em"
            fill={usLabelColor} textTransform="uppercase">
            {h.label}
          </text>
      ...
      {latamHubs.map(h => (
        <g key={h.id}>
          ...
          <circle cx={h.x} cy={h.y} r="5.5" fill={latamDotFill} stroke={isDark ? "var(--navy-1000)" : "white"} strokeWidth="1.8" />
          <text x={h.x} y={h.y + 28} textAnchor="middle"
            fontFamily="Oswald, sans-serif" fontSize="15" fontWeight="500" letterSpacing="0.06em"
            fill={latamLabelColor}>
            {h.label}
          </text>
          <text x={h.x} y={h.y + 44} textAnchor="middle"
      ...
      {/* center caption */}
      <g>
        <text x="600" y="380" textAnchor="middle"
